...
// Quick action types
type QuickActionType = 'VOTE' | 'CONFIRM_PAYMENT' | 'REVIEW' | 'REPLY'
...
function getUrgency(endsAt: Date | null): Urgency {
...
export const quickRouter = router({
  ...
  /**
   * Get all pending quick actions for the current user
   * Returns: pending votes, pending payment confirmations
   * Sorted by urgency (high first)
   */
  getMyActions: publicProcedure
    .input(
      z.object({
        userId: z.string(),
